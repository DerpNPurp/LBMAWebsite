...
Deno.serve(async (req) => {
  ...
  if (!RESENDABLE_STATUSES.includes(lead.status)) {
    return new Response('Lead is not in a resendable state', { status: 422 })
  }
  if (!lead.booking_token) return new Response('Lead has no booking token', { status: 422 })
  ...
  if (notifError) return new Response('Notification failed', { status: 500 })
